...
if ($window) {
    Write-Host "Window found. Clicking Run Demo..."
    ...
    if ($rect.Width -gt 0 -and $rect.Height -gt 0) {
        ...
        $bmp.Save("C:\Users\srini\.gemini\antigravity-ide\brain\aed7e014-82a3-40fd-afb6-9eece0cedb42\landing_page.png", [System.Drawing.Imaging.ImageFormat]::Png)
        $graphics.Dispose()
        $bmp.Dispose()
        Write-Host "Saved landing_page.png"
    }
    ...
    if ($btn) {
        ...
        if ($rect.Width -gt 0 -and $rect.Height -gt 0) {
            ...
            $graphics = [System.Drawing.Graphics]::FromImage($bmp)
            $graphics.CopyFromScreen([int]$rect.X, [int]$rect.Y, 0, 0, $bmp.Size)
            $bmp.Save("C:\Users\srini\.gemini\antigravity-ide\brain\aed7e014-82a3-40fd-afb6-9eece0cedb42\dashboard_demo.png", [System.Drawing.Imaging.ImageFormat]::Png)
            $graphics.Dispose()
            $bmp.Dispose()
            Write-Host "Saved dashboard_demo.png"
        }
    } else {
